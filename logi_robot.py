...
from line_sensor_dual import *
from line_sensor_stm import *
from motor import *
import math

# # Đã có khối để khởi tạo và tự sinh code, nhưng cần liên kết với các hàm trong logi_robot.py
# line_sensor = LineSensorI2C()
# line_sensor1 = LineSensorI2C()
# line_sensor2 = LineSensor2(scl_pin2 = D7_PIN, sda_pin2 = D8_PIN) # Đây là ví dụ về việc nhập PIN

# md_v2 = MotorDriverV2()
# robot = DriveBase(MODE_2WD, m1=None, m2=None, m3=None, m4=None)

# # Đã có khối để khởi tạo và tự sinh code, người dùng sẽ chọn chân, ví dụ M1,M2, E1, E2... và chọn reversed nếu cần
# motor1 = DCMotor(md_v2, E1, reversed=True)
# motor2 = DCMotor(md_v2, E2, reversed=True)
# # ----------------------------------------------------------------------------------------------

# EXTENSION NHẬP CÁC BIẾN PID
Kp_motor = 0
Ki_motor = 0
Kd_motor = 0

# Giá trị mặc định, không cần viết khối
n4 = 0
Error_M1 = 0
huong = 1
Error_M2 = 0
...
# Không cần viết EXTENSION khối cho 2 hàm này:
# Stop the robot
async def stop():
  motor1.run(0)
  motor2.run(0)
